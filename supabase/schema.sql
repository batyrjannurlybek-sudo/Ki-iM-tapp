...
-- ============================================================================
-- ANALYTICS / COMPETITION: views + contacts, leaderboard RPCs
-- ============================================================================
...
create policy "insert events" on events for insert with check (type in ('view', 'contact'));
...
language sql stable security definer set search_path = public as $$
  ...
  from stores s
  left join events e on e.store_id = s.id and e.created_at > now() - make_interval(days => p_days)
...
language sql stable security definer set search_path = public as $$
  ...
  from events
  where store_id = p_store and created_at > now() - make_interval(days => p_days);
$$;
